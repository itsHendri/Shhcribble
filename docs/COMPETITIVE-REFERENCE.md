...
1. **The category has converged on "voice edits existing text", and we don't have it.** FluidVoice ships in-place rewriting via **`RewriteModeService` / Edit mode**; Wispr Flow ships **Command Mode** (voice-edit highlighted text — "make this more concise"); VoiceInk ships a **Rewrite** preset and Aqua an **Edit Mode**. Several independent competitors landed the same idea. *(⚠ **Corrected 2026-08-11 by reading their source:** FluidVoice's **"Command Mode" is not text editing** — it is a full agentic macOS **terminal agent** (`You are an autonomous, thoughtful macOS terminal agent`, `execute_terminal_command` tool calls, `osascript` recipes, `rm` safety rules). The conclusion stands, but the earlier citation pointed at the wrong feature, and at a very different blast radius.)* Every one of our styles transforms *what you just said*; none can act on *what's already on screen*. This is now the clearest capability gap in the product, ahead of anything on the roadmap.
