...
- **Sticky ↔ Notes-pane sync only propagates the FIRST edit** (reported 2026-07-24, **NOT diagnosed**). With a note pinned, editing in one editor updates the other once; subsequent edits in the same session don't land, and the two drift apart again. The earlier root cause — focus tracked from first-responder changes alone, so clicking a floating sticky never made the Studio's editor resign and it refused all store pushes forever — **is fixed** (`2599d11`); this is a *further* failure on top of it, deliberately parked rather than guessed at.
  **Key observation (human, 2026-07-24): unpinning DOES push the pending change through.** Unpin tears the panel down → SwiftUI `onDisappear` → `flushSave()`, which commits *unconditionally*. So the edit is in the editor and the commit path works; what fails is the **debounced save during editing**.
  **Prime suspect — the key-window requirement added in `2599d11`.** `StickyView` only schedules a save when `model.isEditing`, which is now `holdsFirstResponder && windowIsKey`. The sticky is a `.nonactivatingPanel` that calls `NSApp.activate(ignoringOtherApps:)` on focus; if that activation moves key status (even briefly), `isEditing` goes false and **every debounced save is skipped** — leaving only the unconditional flush on teardown. That fits the symptom exactly, including "the first one lands".
  **Recommended fix — stop gating saves on focus at all.** Focus is the right guard for suppressing *store pushes into* an editor, but the wrong one for deciding whether the user typed. `NSTextView` already tells us precisely: `textDidChange` fires for user edits and **not** for a programmatic `textStorage.setAttributedString`. Add an `onUserEdit` callback to `RichTextEditor`, fire it from `textDidChange`, and have both editors schedule their debounced save from that instead of from `.onChange(of: attributed)` + `isEditing`. That removes the whole class of bug rather than re-tuning the focus condition.
