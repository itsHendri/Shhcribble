...
> **NOT prioritized. Re-prioritization is deliberately deferred** (human's call, 2026-07-26): *"we'll re-prioritize features another time — I just want to make sure we've got it all added."* This section is the complete captured findings with pros and cons so the next prioritization session has everything in one place. Nothing here is scheduled; the numbered live pointer above is still the running order.
>
...
| **C2** | **Chunked capture for long recordings** — drain every N s, chunk to disk, transcribe incrementally, overlap + dedup | Fixes a real defect (below), not a feature; also gives partial results if a long capture fails midway | Touches the capture path → **human-gated**. Overlap dedup is fiddly to get right | **Human-gated (audio).** Log as a defect first |
| **C3** | **Two-stream both-sides capture via ScreenCaptureKit** — `SCStream` audio + mic, tagged; mic = "Me", system = "Others" | **Materially simpler than the Core Audio process taps we scoped** for item 6B. Gives free two-speaker attribution with no diarization model. Unblocks the strongest on-brand pitch: "transcribe a call, nothing leaves your Mac" | Costs the **Screen & System Audio Recording** TCC grant — real friction, and the *capability* alone alarms privacy-minded readers of a public diff. Consent/legal note needed. Must stay opt-in, off by default | **Design-gated (human).** Re-scopes item 6B — don't start without re-reading that section |
