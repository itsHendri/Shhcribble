...
- **Sticky ↔ Notes-pane sync only propagates the FIRST edit** (reported 2026-07-24, **NOT diagnosed**). With a note pinned, editing in one editor updates the other once; subsequent edits in the same session don't land, and the two drift apart again. The earlier root cause — focus tracked from first-responder changes alone, so clicking a floating sticky never made the Studio's editor resign and it refused all store pushes forever — **is fixed** (`2599d11`); this is a *further* failure on top of it, deliberately parked rather than guessed at.
  **What's known:** the first sync works, so the observer chain (store `@Published` → `.onChange(of: note)` → `syncFromStore`) is wired correctly and focus is being reported at least once.
  **First things to check, in order:** (1) whether `.onChange(of: note)` fires on the *second* store write at all — `Note` is `Equatable` and includes `richText: Data?`, so an unchanged-looking value would suppress it; (2) whether `lastSyncedText`/`lastSyncedRich` (detail) and `lastAppliedText`/`lastAppliedRich` (panel) go stale after the first round-trip, since both guards compare against them and a mismatch would silently skip every later sync; (3) whether `RichTextEditor.updateNSView`'s `textView.attributedString() != attributed` comparison starts returning false — an `NSAttributedString` that has been through `setAttributedString` and back is not reliably `==` to the value the binding holds. Add logging at all three before changing anything.
  **Not autonomy-safe to guess at:** two live editors over one row, so a wrong fix loses user text. Reproduce with logging first.
