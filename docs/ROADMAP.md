...
- **Sticky ↔ Notes-pane sync only propagates the FIRST edit** (reported 2026-07-24, **NOT diagnosed**). With a note pinned, editing in one editor updates the other once; subsequent edits in the same session don't land, and the two drift apart again. The earlier root cause — focus tracked from first-responder changes alone, so clicking a floating sticky never made the Studio's editor resign and it refused all store pushes forever — **is fixed** (`2599d11`); this is a *further* failure on top of it, deliberately parked rather than guessed at.
  **Key observation (human, 2026-07-24), corrected:** it is **not** unpinning that pushes the change — it's **clicking the sticky's X, and only on the first attempt**. Clicking X moves first responder off the text view, so `resignFirstResponder` fires → `onFocusChange(false)` → `flushSave()`. The commit path therefore works; what fails is every *other* route to a save.
  **Leading hypothesis (coherent with both observations, NOT yet verified) — the key-window requirement added in `2599d11` plus its edge detection.** Focus is now `holdsFirstResponder && windowIsKey`, and `reportFocusIfChanged()` only fires on a *change*. The sticky is a `.nonactivatingPanel`; if `windowIsKey` reads false for it, then:
  1. focus never goes true → `model.isEditing` stays false → **every debounced save is skipped**;
  2. and because focus was never reported true, the later `resignFirstResponder` finds no edge → **`onFocusChange(false)` never fires → `flushSave()` never runs either**.
  That explains "first attempt only" precisely: the very first focus report happens while the panel genuinely is key, so that one X click flushes; afterwards `windowIsKey` is false, no edge is ever recorded again, and nothing saves.
  **Also flagged while reading this path — a data-loss risk, not just a sync gap:** `update(with:)` refuses store pushes on `!model.isEditing`. If `isEditing` is wrongly false, an incoming store change will **overwrite text the user is actively typing**. Focus must not be the guard for that either.
  **Recommended fix — take focus out of the correctness path entirely.** It is currently load-bearing for two unrelated questions, and when it is wrong *both* break (one of them destructively):
  - *"Should we save?"* → should be **"did the user type?"**. `NSTextView` answers exactly that: `textDidChange` fires for user edits and **not** for a programmatic `textStorage.setAttributedString`. Add an `onUserEdit` callback to `RichTextEditor`, fire it from `textDidChange`, and schedule the debounced save from that.
  - *"May we push store content in?"* → should be **"is there an uncommitted user edit here?"**. Track a `hasPendingUserEdit` flag, set on `textDidChange` and cleared once the commit lands; refuse pushes while it is set.
  Focus then only remains for `NSApp.activate` (so ⌘V works in a sticky), where being wrong is harmless. Re-tuning the focus condition instead would leave both failure modes one edge case away.
  **Still don't fix by inspection:** two live editors over one row, so verify with logging on both save paths before and after.
